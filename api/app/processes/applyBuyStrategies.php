...
while (true) {
  ...
  try {
    ...
    if (!$run_before && $status == 'Open') {
      try {
        StockLogger::emptyStock('Buy');
        foreach ($config['buy'] as $i => $run) {
          ...
          $process = new Process("BuyStrategy", "$log_dir/run-$i.log");
...
function restartTomorrow()
{
  ...
  $new_process = new Process("applyBuyStrategies");
  $new_process->setLogger(APP_DIR . '/processes/logs/Buy-' . (new Ndate)->format() . '.log');
  if ($new_process->run(Process::BACKGROUND)) {
    $json = json_decode(file_get_contents(JSONS_DIR . "/Buy.json"), true);
    $json['pid'] = $new_process->getPID();
    file_put_contents(JSONS_DIR . "/Buy.json", json_encode($json));
    exit;
  }
}
