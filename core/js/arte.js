...
function display(text) {
  ...
  const lowercaseWords = ['of', 'in', 'off', 'over'];
  return text.replaceAll('-', ' ').replaceAll('_', ' ').toLowerCase().replace(/\b\w+\b/g, function (word) {
    return lowercaseWords.includes(word) ? word : word.charAt(0).toUpperCase() + word.slice(1);
  });
}

function counter(selector, value) {
  $({ someValue: 0 }).animate({ someValue: Math.floor(value) }, {
    duration: 3000,
    easing: 'swing', // can be anything
    step: function () {
      $(selector).text(commaSeparateNumber(Math.round(this.someValue)));
    },
    complete: function () {
      $(selector).text(commaSeparateNumber(Math.round(value)))
    }
  });
...
const local = {
   ...
   */
  get: function (key) {
    key = this.encrypt(`${config.APPNAME}-${key}`);
  ...
  getAll: function (keys = []) {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      key = this.decrypt(key).split(config.APPNAME + '-')[1]
  ...
  getCookie: function (key, encrypt = false) {
    if (encrypt) key = this.encrypt(`${config.APPNAME}-${key}`);
    else key = `${key}`;
...
class AJAX {
  ...
  static ajax(options) {
    ...
    if (options.complete !== undefined) {
      let callbacks = { ...options.complete }

      options.complete = function (xhr, status) {
        AJAX.complete(xhr, callbacks, options.url, options.data);
      };
    }
...
class Form {
  ...
  initialize() {
    ...
    if (!this.submitBtn.length)
      error("submit btn not found for " + this.title + " form");
...
class Sender {
  ...
  constructor(form) {
    ...
    if (this.endpoint === undefined)
      error("api destination not found for " + this.form_title + " form");
    this.type = $(form).attr("request_type") ?? $(form).attr("type") ?? "GET";
  }
...
class Validator {
  ...
  constructor(input) {
    ...
    if (!$(input)[0].hasAttribute("name"))
      error(`Validation Creation Error: input ${this.fieldName} has no attribute name inside form ${$(input).parents("form").attr("id")}`);
  }
  ...
  run() {
    ...
    if ((input.length || $(this.input)[0].hasAttribute("must")) && $(this.input)[0].hasAttribute("validation")) {
      ...
      input = this.val() // read again in case the validation changed the input value
    }
...
class Validators {
  ...
  showErr(validator, err) {
    ...
    $(this.msgContainer).html(display(text));
  }
}
...
class Table {
  constructor(table, cols) {
    ...
    if (Array.isArray(cols)) {
      ...
      error('Invalid Cols Passed: ' + cols);
  ...
  parseHTML() {
    this.cols = {}
    this.$('thead tr th, thead tr td').each((i, item) => {
      const text = $(item).text().trim();
      this.cols[text.replaceAll(' ', '_').toLowerCase()] = text;
    });
  ...
  init() {
    this.table.html(`
      ${this.createTHead(this.createTR())}
      ${this.createTBody()}
    `)
    ...
    for (let col in this.cols)
      this.$('thead tr').append(this.createTH(col))

    this.inited = true
  }

  createTHead(content = '', attrs = '') {
    return `<thead ${attrs}>${content}</thead>`;
  }

  createTBody(content = '', attrs = '') {
    return `<tbody ${attrs}>${content}</tbody>`;
  }

  createTH(col, display = '', attrs = '') {
    if (display === '')
      display = this.cols[col]
    return `<th class="col-${col}" ${attrs}>${display}</th>`
  }

  createTD(col, value, attrs = '') {
    let data_value = Regex.GENERIC.test(value) ? `data-value="${value}"` : ''
    return `<td class="col-${col}" ${data_value} ${attrs}>${value}</td>`
  }

  createTR(content = '', attrs = '') {
    return `<tr ${attrs}>${content}</tr>`;
  }

  $(something) {
    return this.table.find(something)
  }

  addRecord(obj) {
    if (!this.inited) {
      error('table not inited')
      return;
    }

    if (this.$('.empty-row').length)
      this.reset()

    if (typeof obj == 'string')
      this.tbody.append(this.createTR(obj));

    else if (Array.isArray(obj)) {
      this.tbody.append(this.createTR('', ''))
      let i = 0;
      for (let col in this.cols) {
        this.tbody.find("tr:last").append(this.createTD(col, obj[i]))
        if (/^[0-9]+$/.test(obj[counter]))
          counter(this.tbody.find("tr:last td:last"), obj[i])

        i++;
      }
    }
    else if (typeof obj == 'object') {
      this.tbody.append(this.createTR('', obj.id ? `data-id=${obj.id}` : ''));

      for (let col in this.cols) {
        if (col in obj) {
          this.tbody.find("tr:last").append(this.createTD(col, obj[col]))
          if (/^[0-9]+$/.test(obj[col]))
            counter(this.tbody.find("tr:last td:last"), obj[col])
        }
        else
          this.tbody.find("tr:last").append(this.createTD(col, '—'))
      }
    }
    else console.warn(`Weird Record Detected at ${this.tbody.data("id")} which is ${obj}`)
  }

  reset() {
    if (!this.inited) {
      error(`table ${this.table.attr('id')} not inited`)
      return;
    }
  ...
  empty(sentence) {
    ...
    this.tbody.html(this.createTR(this.createTD('', sentence, `style="color:red" class="empty-row" colspan="${Object.keys(this.cols).length}"`)))
  }
  ...
  filter(needle, col) {
    ...
    let keys
    if (col) {
      keys = Object.keys(this.cols)
      for (let i in keys)
        if (col == keys[i])
          col = i
    }

    this.$("tbody tr").each(function (i, tr) {
      let haystack
      if (haystack)
        haystack = $(tr).find(`td:nth(${col})`).text().toLowerCase()
      else
        haystack = $(tr).text().toLowerCase()
